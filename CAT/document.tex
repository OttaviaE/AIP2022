...
\begin{frame}{Item Response Theory}{2-PL Model}
				...
				P(x_{ps} = 1|\theta_p, b_j, a_j) = \frac{exp[a_j(\theta_p - b_j)]}{1 + exp[a_j(\theta_p - b_j)]}
			\end{equation*}
		\vspace{3mm}
			where: 
		
			$P(x_{ps} = 1)$: Probability of a correct response to item $s$ by respondent $p$
			...
			$\theta_p$: Ability of respondent $p$
				...
				\vspace{1.5mm}
			$b_j$: Difficulty of item $s$
			
				\vspace{1.5mm}
			$a_j$: Discrimination of item $s$
...
\begin{column}{.50\linewidth}
	...
	\mathit{TIF} = \displaystyle \sum_{s=1}^{S} \mathit{IIF}_j
\end{equation*}
\vspace*{-6mm}
\pause
\begin{figure}
